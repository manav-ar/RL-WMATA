...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train DQN agent for station placement")
    parser.add_argument(
        "--total_timesteps",
        type=int,
        default=10000,
    ...
    # DQN requires learning_starts timesteps before learning begins
    # For very small test runs, adjust learning_starts to avoid hanging
    default_learning_starts = 1000
    if total_timesteps < default_learning_starts:
        # Ensure learning_starts is at least 100 (DQN minimum)
        adjusted_learning_starts = max(100, total_timesteps // 2)
        print(f"⚠️  Warning: total_timesteps ({total_timesteps}) is less than learning_starts ({default_learning_starts})")
        print(f"   Adjusting learning_starts to {adjusted_learning_starts} for this test run")
    ...
    if torch.cuda.is_available():
        batch_size = 64  # Reduced from 128 - faster training
        buffer_size = 20000  # Reduced from 50000 - faster sampling
        policy_kwargs = dict(
    ...
    # Define DQN model with GPU-optimized settings
    ...
    if use_viz and total_timesteps < 50000:
        ...
        viz_callback = VisualizationCallback(
            ...
            network_path=NETWORK_PATH,
            station_od_path=STATION_OD_PATH,
            save_dir="visualizations/training/dqn",
    ...
    print(f"   learning_starts={learning_starts}, batch_size={model.batch_size}, buffer_size={model.buffer_size}, n_envs={n_envs}")
    ...
    model.save("models/dqn_station_placement")

    print(f"✅ Training complete. Model saved to models/dqn_station_placement.zip")
